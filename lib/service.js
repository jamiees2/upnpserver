...
var debug = require('debug')('upnp:service');
...
Service.prototype.processRequest = function(request, response, path, callback) {
  ...
  // debug("Unknown request url '" + path + "'");
...
Service.prototype.processControlRequest = function(request, response, path,
  ...
  if (!fn) {
    debug("processControlRequest: Unknown soap function 'processSoap_" + soapAction + "'");
...
Service.prototype.processEventRequest = function(request, response, path,
  ...
  console.log("Invalid request method " + request.method); // Error?
...
Service.prototype.processUnsubscribe = function(request, response, callback) {
  ...
  if (!client) {
    debug("Unsubscribe: Unknown client uuid=", uuid);
    return callback(null, true);
  }
  ...
  debug("Unsubscribe: Delete client uuid=", uuid, " url=", client.url);
...
Service.prototype.processSubscribe = function(request, response, callback) {
  ...
  debug("Subscribe: Client uuid=", uuid, " url=", callbackURL);
...
Service.prototype.sendEvent = function(eventName, xmlContent) {
  ...
  // debug("Send event xmlContent=", xmlContent);
  ...
  // debug("Send event ", xml, " to " + clientsCopy.length + " clients");
  ...
  Async.eachLimit(clientsCopy, EVENT_CLIENTS_PROCESSOR_LIMIT, function(client,
    ...
    try {
      ...
      console.log("Can not send http request", x); // Error?
    }
  });

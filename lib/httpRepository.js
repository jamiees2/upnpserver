...
var debug = require('debug')('upnp:http');
...
HTTPRepository.prototype.browse = function(list, item, callback) {
  ...
  debug("HTTPRepository: browseItem=" + itemPath);
  var path = itemPath.substring(this.mountPath.length);
  if (path !== "") {
    if (path[0] === "/")
      path = path.substr(1);
    var sp = path.split("/");
    Async.reduce(sp, this.files, function(files, p, cb){
    ...
    return this.browseFiles(list, item, this.files, callback);
  }
};

HTTPRepository.prototype.browseFiles = function(list, item, files, callback) {

    debug("HTTPRepository: returns " + files.length +
        " files");
  ...
  Async.reduce(files, [], function(list, file, callback) {
    ...
    debug("HTTPRepository: END browse=", item.getPath());
    callback(null, list);
  });
};

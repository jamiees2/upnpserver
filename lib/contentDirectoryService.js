...
var debug = require('debug')('upnp:contentdirectory');
...
ContentDirectoryService.prototype.allocateItemsForPath = function(path,
  ...
  Async.reduce(ps, this.root, function(parentItem, segment, callback) {
    ...
    parentItem.getChildByName(segment, function(error, item) {
      ...
      if (item) {
        // debug("allocateItemsForPath(" + segment +
        // ")=>",item.id);
      ...
      // debug("allocateItemsForPath(" + segment+ ")=> NEW
      // CONTAINER");

      self.newContainer(parentItem, segment, null, true, {
        virtual : true
      }, callback);
    });
  }, callback);
...
ContentDirectoryService.prototype.responseObject = function(response, request,
  ...
  debug("Request ObjectId=" + objectId);
...
ContentDirectoryService.prototype.responseContainer = function(response,
  ...
  debug("Request containerId=" + containerId + " filter=" + filter +
  ...
  this
      .getItemById(
          ...
          function(error, item) {
            ...
            if (error) {
              console.log("CDS: Can not getItemById for id", containerId); // Error
              return callback(error);
            }
            if (!item) {
            ...
            item
                .listChildren(function(error, list) {
                  if (error) {
                    debug("Can not scan repositories: ", error);
                    return callback(error);
                  }
                  ...
                  Async.eachSeries(list, function(item, callback) {
                    ...
                    item.toJXML(repositoryRequest, function(error, xml) {
                      if (error) {
                        return callback(error);
                      }
...
ContentDirectoryService.prototype.browseItem = function(item, callback) {
  ...
  debug("CDS: browseItem itemID=" + item.id + " path='" + path +
  ...
  Async.reduce(this.repositories, [], function(list, repository, callback) {
    ...
    if (path.indexOf(repository.mountPath) !== 0) {
      ...
      debug("CDS: browseItem repository mountPath=" +
    ...
    repository.browse(list, item, function(error, result) {
      if (error) {
        console.log("CDS: browseItem repository mountPath=" +
            repository.mountPath + " error ", error); // Error
        return callback(error);
      }
    ...
    if (error) {
      console.log("CDS: browseItem '" + path + "' returns error ", error); // Error
      return callback(error);
    }

    debug("CDS: browseItem '" + path + "' returns " + list.length +
        " elements.");
...
ContentDirectoryService.prototype.newItem = function(parent, name, upnpClass,
  ...
  this.registerItem(item, function(error) {
    if (error) {
      console.log("Register item error=", error); // Error
      return callback(error);
    }
...
ContentDirectoryService.prototype.newFile = function(parent, path, upnpClass,
  ...
  self.computeTitle(name, path, mimeType,
      function(error, title) {
        ...
        if (self.upnpServer.dlnaSupport) {
          attributes.resAttrs.protocolInfo += "DLNA.ORG_OP=01;DLNA.ORG_CI=0;DLNA.ORG_FLAGS=017000 00000000000000000000000000";
        } else {
          attributes.resAttrs.protocolInfo += "*";
        }

        function waitStats(stats) {
          if (!self.upnpServer.configuration.strict) {
...
ContentDirectoryService.prototype.updateItem = function(item, callback) {
  ...
  debug("CDS: updateItem itemID=" + item.id + " path='" + path +
      "' repositories.count=" + this.repositories.length);
  ...
  Async.each(this.repositories, function(repository, callback) {
    ...
    if (error) {
      console.log("CDS: updateItem '" + path + "' returns error ", error); // Error
      return callback(error);
    }
...
ContentDirectoryService.prototype.removeItemById = function(id, callback) {
  this._itemRegistry.removeItemById(id, callback);
};
...
ContentDirectoryService.prototype.processRequest = function(request, response,
  ...
  if (path.indexOf(contentPath) === 0) {
    ...
    return this.getItemById(id, function(error, item) {
      ...
      if (!item || !item.id) {
        console.log("SendItem itemId=", id, " not found"); // Error
